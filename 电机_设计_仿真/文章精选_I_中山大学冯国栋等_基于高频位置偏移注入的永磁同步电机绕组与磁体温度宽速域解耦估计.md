# 文章精选 I 中山大学冯国栋等：基于高频位置偏移注入的永磁同步电机绕组与磁体温度宽速域解耦估计

原创 CES TEMS编辑部 CES电机与系统学报 2026-03-09 07:00 北京
...
Chengtao Shi1; Yuting Lu1; Kaide Huang2; Chunyan Lai3; Beichen Ding1; Guodong Feng\*1  
...
在永磁同步电机（PMSM）驱动器中，温度信息对于实现可靠和高性能的控制至关重要。流行的基于模型的估计方法是通过使用机器模型从电压中提取温度相关项。由于低信噪比，模型的不确定性和噪声会极大地影响低速或负载下的估计精度。本文提出了一种基于高频（HF）位置偏移注入的永磁同步电机绕组和永磁体（PM）温度解耦估计方法，以实现在宽速度范围内，特别是在低速条件下的准确和鲁棒的温度估计。在所提出的方法中，将一个小的HF位置偏移注入机器中，以构建一个解耦的绕组和PM温度估计模型，其中绕组和PM的温度是从HF激励中独立估计的。温度估计独立于基本模型和参数变化，在低速条件下实现了高信噪比。此外，温度估计也不受磁饱和和逆变器失真的影响，这可以提高温度估计的准确性和鲁棒性。通过在不同操作条件下在实验室机器上进行的实验和比较，验证了所提出的方法。  
