...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_000_87835d916702.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_001_0b391399449b.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_002_d1b14aa092dc.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_003_3323993f7924.png)

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_004_31a38cc31c23.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_005_77952b48c868.png)

典型的 SiC MOSFET 开通瞬态波形如图 3 所示。开通瞬态过程可分为六个阶段，各阶段内的开关器件状态、已知条件与近似假设见表 1。

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_006_8237c82e799e.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_007_3eb678549f08.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_008_a83d35550d18.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_009_3d02b2c8030e.png)

式中，Lstray 为主回路杂散电感。

（1）当α(on3)＜ω0(on3)时，id 呈欠阻尼规律变化，其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_010_f1e6435cc50a.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_011_0de81733ad59.png)

（2）当α(on3)＞ω0(on3)时，id 呈过阻尼规律变化，其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_012_935660f44578.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_013_4a4c2d1540ba.png)

该阶段内，栅源电压 vgs 对 id 的控制关系可用跨导 gfs 表示，即

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_014_223e4b7dfbd6.png)

由于杂散电感与杂散电阻上的压降，vds 出现小幅下降，即

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_015_2b72f3a0ac1a.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_016_981780c9905f.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_017_6f7733bda2c7.png)

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_018_e690ec505d6d.png)

tp 时刻，did/dt =0，由此可解得 id 峰值时间 tp与峰值 Ipeak 分别为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_019_399c5eed18be.png)

（2）\[tp, t4\]，vds 快速下降，id 由峰值 Ipeak 开始下降，vgs 近似为 Vmiller 保持不变，可认为|dvgs/dt|＜＜|dvds/dt|。 id 一般呈欠阻尼规律变化， 其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_020_23691b9d2dfb.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_021_8908bc824d97.png)

dvds/dt 表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_022_f7fc951ab79e.png)

该阶段内栅漏极电容 Cgd 近似等于其 PN 结耗尽层电容 Cgdj，其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_023_73352321a92d.png)

式中，Vdgj0 与γgdj 为非线性电容模型的两个参数。

由于 vdg≈vds，可将式（23）代入式（22）得到该阶段内 vds 的解析表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_024_e6e34f55d371.png)

由于 vds 快速变化，输出电容 Coss 的充电电流 ic不可忽略，vgs 的表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_025_b72e02543ac6.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_026_dc9140fecbd7.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_027_c5bfed01d51b.png)

该阶段内，可假设 vgs 近似保持米勒电平 Vmiller不变，则 vds 表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_028_362223a4f373.png)

6）阶段六\[t5, t6\]，id 拖尾振荡

t5 时刻后，id 拖尾振荡，变化规律与阶段五相同，其表达式如式（26）所示。近似认为 vds=Rds(on)IL保持不变。vgs 继续以指数方式上升至 VCC，其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_029_4c2816c6a2cb.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_030_03f67cc12686.png)

典型的 SiC MOSFET 关断瞬态波形如图 4 所示。关断瞬态过程可分为六个阶段，各阶段内的开关器件状态、已知条件与近似假设见表 2。

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_031_f51a375d405c.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_032_9a486a386f8e.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_033_40d87e1356e5.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_034_d4689d60ea96.png)

该阶段内，vjD 迅速下降，SBD 结电容放电电流iDiode 不可忽略，导致 id 缓慢下降，近似认为 dvjD/dt=−dvds/dt，则 id 的表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_035_62fec2c0406e.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_036_e806ccaa8eee.png)

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_037_e93b5f3e6e50.png)

其中

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_038_6700d9180729.png)

（1）当α(off5)＜ω0(off5)时，id 呈欠阻尼规律变化，其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_039_a7e49f2fdc93.png)

（2）当α(off5)＞ω0(off5)时，id 呈过阻尼规律变化，其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_040_987974d4ca83.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_041_718ea8f0c379.png)

  
vgs 继续以指数方式下降至 VEE，其表达式为

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_042_897e84fcae11.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_050_a3880efd30f6.png)

为便于模型计算，以分段电容值反映寄生电容Cds 与 Cgd 的非线性，即

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_051_30843dba6311.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_052_437c2ef101e6.png)

由式（5）可根据 datasheet 结电容曲线拟合相关参数。为便于模型计算，以分段电容反映 SBD 结电容 CjD 的非线性，即

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_053_fbbd97d87d61.png)
...
![](D:\电脑文件\公众号知识库\电力电子_宽禁带器件_SiC_GaN\SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images\img_000_e795ab943411.png)
...
![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_055_565d30aae5d2.png)

3.2 模型验证

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_056_173bfe06facc.png)

改变驱动回路外接电阻 Rg(ext)，分别将理论计算与实验测量所得的开关瞬态波形与开关损耗进行对比，如图 8 与图 9 所示。

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_057_f7f8a897d4a3.png)

改变SiC MOSFET栅漏两极外接电容 Cgd(ext)，分别将理论计算与实验测量所得的开关瞬态波形与开关损耗进行对比，如图 10 与图 11 所示。

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_058_80e35a60cb27.png)

![](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_059_b4fa421c1774.png)
...
![图片](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_060_6f3582a6655d.jpg)

    专注碳化硅器件的研发与应用。分享碳化硅器件的设计@研发@应用等行业资料。

  加交流微信群，请添加个人微信：18126115420，并备注单位+姓名+研发方向。

![图片](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_061_433ba14f0454.jpg)

![图片](SiC_MOSFET_与_SiC_SBD_换流单元瞬态模型_images/img_062_7038903c64cb.png)