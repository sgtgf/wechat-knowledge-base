...
原创 自动化学报 2019-11-29 17:30 北京

> 原文地址: [https://mp.weixin.qq.com/s/uyXsmMZh0g8Aifw-wfVLYg](https://mp.weixin.qq.com/s/uyXsmMZh0g8Aifw-wfVLYg)
...
龚军辉, 刘小燕, 周建松, 孙刚. 基于时空双稀疏表示的成人ADHD脑网络检测与分析. 自动化学报, 2019, 45(10): 1903-1914.

  

注意力缺陷多动障碍(attention deficit hyperactivity disorder, ADHD)是一种常见的精神障碍疾病, 主要表现为与发育程度(年龄) 不相符的注意力分散、多动和冲动, 这种疾病往往开始于童年并可能持续到成年。ADHD对患者的生活和学习产生不利的影响, 且成人ADHD 患者更易引发其他的精神障碍与犯罪行为, 因而是一个重要的公共卫生问题。静息态功能核磁共振成像由于操作易、耗时短、性价比高等优点，被广泛应用于探索青少年和成人ADHD患者的脑功能异常，传统的ADHD静息态功能核磁共振成像研究主要分为两类：

  

(1) 一些学者采用局部一致性、低频振荡等基于体素的指标分析了静息态fMRI数据，认为ADHD患者存在一些异常脑区，例如运动感知脑区，前额、前扣带回和丘脑，楔叶、楔前叶和脑岛等脑区。

  

(2) 另一些学者则从各脑区（或体素）之间的功能连接角度出发对ADHD静息态fMRI数据进行了分析，发现ADHD患者的前扣带回与楔前叶及后扣带回、楔前叶与默认网络的其他脑区有所减弱等。

  

虽然上述研究给出了一些与ADHD相关的脑区或脑区间的功能连接，但是ADHD的潜在生理病理机制目前仍不完全明了，需进一步研究验证。本文设计的基于时空双稀疏表示的成人ADHD相关静息态脑网络检测方法，首先采用DTSSR从ADHD的静息态fMRI数据中提取出组脑网络及相应的耦合参数；然后将耦合参数平均池化作为网络的活跃度指标；最后，将活跃度指标与ADHD的量表分进行Spearman相关性分析，检测出与ADHD相关的脑网络。设计的方法整体框图如下：　
...
采用上述方法从大尺度脑网络角度探讨成人ADHD潜在的神经机制，主要结论和发现有：

  

(1) 一些正常人组上常见的静息态脑网络，在采用时空双稀疏表示方法从ADHD组静息态fMRI数据获得的静息态脑网络中，并不缺失，且两者相对应的静息态脑网络之间具有较高的相关性系数和显著相关性，在空间分布上也高度相似。

  

(2)对脑网络活跃度指标与ADHD量表分进行Spearman相关性分析，背侧注意网络和执行控制网络(如图2)与ADHD显著相关。这个结果从脑科学角度有合理的解释，在前期的研究中也能找到相应的支持依据。

  

(3)采用所提方法和指标获得的背侧注意网络和执行控制网络，在不同字典尺寸下具有较高的稳定性。
...
E-mail：gangsun@hnu.edu.cn@163.com
