...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_000_f0abced6f6dd.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_001_1ac163510985.png)

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_002_0721f3ac645d.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_003_5b1456b0c82a.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_004_c2ebd5fb719a.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_005_8a14ddbf17ea.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_008_fc20dab02133.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_009_088d2b86a987.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_010_e1a3f4421c16.png)

此外，每一步结束后都应在显微镜下观察是否有芯片损伤，并测量各引脚的电阻是否匹配。功率模块的封装效果如图11 所示，在应用前，应对模块的封装性能进行测试。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_011_ab2fb564d42e.png)

2. 功率模块的对比评估

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_012_89b574d5f8e7.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_013_9c4fedc2263c.png)

3 种器件组合在相同封装结构下的性能差异性还未见文献报道。因此，在相同封装条件下，需要评测全SiC、混合及全Si 模块的性能、适用场合及经济性，为功率模块的应用设计提供参考。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_014_dc53351ebe97.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_015_303a7b1fdb95.png)

式中τ 为时间常数，τ =RG(CGS+CGD)，其中RG 为驱动电阻，CGS、CGD分别为栅–源电容和米勒电容。

uGS从0 到VTH的时间(t1)为开通延迟时间，有：

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_016_342c8fa61709.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_017_f0c5025e7d68.png)

式中：gm为跨导；k 为与器件制造相关的常数。

iD 以非线性平方关系随时间增加，如图13 所示。持续时间与负载电流相关，有：

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_018_ba3fcd5dcef2.png)

3）第三阶段(t2—t3)，iD\=IL，漏–源电压uDS 由VDS减小至正向导通电压VF，下降速率duDS/dt 为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_019_21e69143468e.png)

该阶段iG 全部为米勒电容充电，uGS、iG 分别维持在米勒平台电压VGP和电流IGP，有：

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_020_19654859c895.png)

第三阶段持续时间为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_021_ffc320ad53f1.png)

由于最大损耗发生在电流峰值IL 和电压峰值VDS处，则开通损耗EON可表示为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_022_83498719cf11.png)

由于SiC SBD 的反向恢复电流可以忽略，则SiC MOSFET的开通损耗近似为全SiC 模块的开通损耗。此外，SiC MOSFET的开关速度快，开通电流的非线性部分可做线性化处理。全SiC半桥功率模块的开通损耗EON可近似为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_023_7308ff817542.png)

式中tri\=t3−t1 为开通时间。

类似地，关断损耗EOFF可表示为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_024_dd194821798d.png)

将电压线性化处理，EOFF可以表示为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_025_761dcf2b48a2.png)

式中tfu\=t6−t4 为关断时间。

因此，全SiC 半桥功率模块的开关损耗ESW可近似表示为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_026_5ef02428cef4.png)

减小器件的输入电容、驱动电阻，降低负载电流以及直流电压，均利于减小瞬态功率及缩短开关时间，从而降低SiC 器件的开关损耗。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_027_17fcf42bfa38.png)

基于图14 所示的双脉冲测试电路，分别测试不同温度、电流等级下功率模块的性能。在VDS\= 600V、RG\=20Ω、结温温度25℃的测试条件下，3种功率模块的实验结果如图 15 所示，

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_028_ee742c696655.png)

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_029_c750e60339c4.png)

负荷电流40A时的动态对比结果如图 16 所示，

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_030_ca2b34ad73ec.png)

损耗对比结果如图17 所示。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_031_c733a7144a46.png)

如图16(c)所示，由于全SiC 模块封装寄生参数与开关速度不匹配，开关过程振荡不可忽略。振荡频率fr为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_032_5bb478b5acff.png)

式中：Lσ 为封装寄生电感；Coss 为器件的输出电容。根据振荡频率26MHz，可计算自主封装功率模块的寄生电感为24nH。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_033_149371317bd1.png)

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_034_667984a45c66.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_035_8078daeb9c84.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_036_cd8c2707c594.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_037_ff1ee5dc285e.png)

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_038_39720a8a310d.png)

由图22 可以看出，不同的封装结构对全SiC 模块损耗有显著影响。从另一种角度来看，为了实现高品质的功率模块，从优化封装结构及材料选择入手，远比改善芯片性能经济有效。因此，迫切需要先进的封装手段，以配合芯片性能，达到整体性能最优。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_039_dd10040627ac.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_040_d40921aa49fa.png)

3.1 热击穿失效

热击穿失效是最常见的失效模式，主要是高密度电流的热效应导致的。根据芯片损耗PG和封装的耗散功率PO之间的平衡关系，芯片升温的条件为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_041_3a40ad2b732b.png)

假定芯片特定位置的散热条件用热阻Rth表示，可定义热稳定系数S 为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_042_009a962a7f7c.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_043_84b2c870bfd5.png)

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_044_dda2ec7400b0.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_045_0d29c68376a1.png)

瞬态过电流可能由电流尖峰或浪涌电流造成， 通常与模块的设计或封装电感有关，特别是键合线接触电阻的作用，常表现为引脚附近单点或多点烧毁，如图25 所示。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_046_68b19c2c0fa4.png)

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_047_5baf0f744174.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_048_b79e952569ef.png)

式中：d 为电流通道的宽度；2D 为芯片的宽度。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_049_72699db7f21d.png)

假设电流的拖尾时间为tf，di(t)/dt\=−IL/tf，i(0)\=IL，则x 方向的磁场恒为零(Hx\=0)，z 方向磁场变化率为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_050_64854a68352a.png)

根据麦克斯韦方程，有：

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_051_4cf2831bca68.png)

式中μ 为半导体材料的磁导率。

若x\=D 处场强Ey(x\=D)\=ED\=VDS/s(其中s 为芯片厚度)，在与原点距离为x 处的场强Ey表示为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_052_720e2da89a3a.png)

因此，芯片上不同的点承受不同的关断电压。 在x\=d 处，有式(24)成立。

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_053_d7672ee67784.png)

对于开通过程，重新建立坐标原点如图26(b)所示，同样可得式(20)、(21)，替换边界条件di(t)/dt\=IL/tf，i(0)\=0，Ey(x\=D)\=ED\=0，可将式(23)改写为

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_054_7234f6233e33.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_055_0e6a03ef5d5f.png)
...
![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_056_6a1b849a6cf8.png)

![](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_057_eb3d2cdf4896.png)
...
![图片](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_058_389a32ed7e2f.jpg)
...
![图片](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_059_5c5a8d468409.jpg)

![图片](SiC_Si_混合功率模块封装对比评估与失效分析_images/img_060_573ddf34e793.png)