...
原创 自动化学报 2020-04-13 17:30 北京

> 原文地址: [https://mp.weixin.qq.com/s/FVDVaBRxvGjg9uDKCi\_Kgg](https://mp.weixin.qq.com/s/FVDVaBRxvGjg9uDKCi_Kgg)
...
**笔迹鉴别**是指：通过机器学习方法对手写文字信息进行关键特征提取，以鉴别书写人身份的一种文件检验技术。
...
陈使明, 王以松. 一种鲁棒的离线笔迹鉴别方法. 自动化学报, 2020, 46(1): 108-116.

  

笔迹鉴别作为机器视觉与模式识别领域的研究热点之一，与人脸识别、语音识别、指纹识别等技术一样属于身份识别的研究范畴，在最近几十年受到广泛关注.它在司法鉴定、历史文档分析、商务安全等方面发挥着重要作用. 

  

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46VBVxYX7bguVhjwM8sQm8Up3aJVT9vLTq9ngBygUIuwmUIbEa9SP56swPicCpzCibNNFepzfDoyGjQ/640?wx_fmt=png)

司法鉴定

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46VBVxYX7bguVhjwM8sQm8UJNHP5T7iaxxK70iau1ASFYymKaepNia0b7ic2U5cbJAUvNZow7PqvIoa0Q/640?wx_fmt=png)

历史文档分析

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46VBVxYX7bguVhjwM8sQm8Ukl3Q55UgJyibEESiaoQ3WTutXzA1BiatWPicmicU6f3Kt9BwWm8W1wrRa1w/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46VBVxYX7bguVhjwM8sQm8UNJCnKw27fE1LhLXTucTQ39w2YINrwZUvYLIQozzV6IXWB4LTweIMGw/640?wx_fmt=png)

DLS-CNN框架图

  

依据笔迹材料的获取形式可将笔迹鉴别分为两种，分别为在线笔迹鉴别与离线笔迹鉴别。在线笔迹鉴别可通过在线采集书写的速度、角度、书写者所使用的力量以及写字的笔画顺序等丰富信息进行笔迹鉴别。离线笔迹鉴别材料使用传统的书写工具在纸张上书写笔迹信息，再通过照相机或扫描仪采集为图片的形式。本文研究的对象即为离线笔迹鉴别。然而，离线笔迹鉴别存在如下几个客观因素：

  

1）随着年龄的增长，书写人的书写笔迹可能会有一定的变化；

2）同一个书写人的材料可能来源于不同笔的使用；

3）书写者的书写物理环境的变化会影响书写者的书写习惯;

4）每个书写者的笔迹材料与笔迹信息都非常有限。

  

以上几个客观因素使得笔迹鉴别的研究极具挑战性。当前的笔迹鉴别方法依赖于数据增强，并需要较多的笔迹信息方可取得较好的识别效果。主要原因在于：

  

1）主流的基于局部特征提取的离线笔迹鉴别方法的在数据预处理阶段提取的小像素快不准确，使得很多完整的笔迹信息被错误分割；

2）当前基于局部特征提取的离线笔迹鉴别方法的模型表征能力不强，使得学习的特征还不能充分适应笔迹鉴别任务。

  

为解决以上问题，并且鉴于图像分割方法准确性与CNN在笔迹鉴别领域的成功应用，我们提出了一种基于统计的文档行分割与深度卷积神经网络方法（Document Line Segmentation-Convolutonal Neural Network ，简称DLS-CNN）。利用基于统计的文档行分割方法分割的准确性以及深度神经网络的强表征能力，学习具有强鲁棒性的局部特征，使得模型在笔迹检索任务中无需依赖于数据增强并通过取均值的编码方式就能够取得较好的鉴别效果，在笔迹识别中仅需要少量的笔迹信息即可对笔迹进行较好地识别.在ICDAR2013\[3\]与CVL\[30\]两个标准数据集上实验结果表明：1）DLS-CNN相较于其他相似方法提取的局部特征具有更强的鲁棒性；2）DLS-CNN只需要极少的笔迹信息就能对笔迹进行较好的识别，模型具有较强的泛化能力.

