...
原创 自动化学报 2020-05-18 18:04 北京

> 原文地址: [https://mp.weixin.qq.com/s/e2JMpML3oIzKORg555ewhQ](https://mp.weixin.qq.com/s/e2JMpML3oIzKORg555ewhQ)
...
Buck-Boost矩阵变换器(Buck-Boost Matrix Converter, BBMC)是一种具有高电压传输比且可直接输出高品质正弦波的新型电力变换器，适合作为功率变换器应用于异步电机调速系统中；而对其研究有效的调速控制策略，则是实现其高性能调速控制的基本前提。
...
刘继, 张小平, 张瑞瑞. 基于 FTC 的 BBMC 调速控制策略及参数优化. 自动化学报, 2020, 46(2): 332−341.

  

鉴于目前针对BBMC所研究采用的各种控制策略，均属于解决无限时间稳定性的控制问题；而对于基于BBMC的异步电机调速系统来说，当其状态发生突变或受到外部扰动时，如何在有限时间内使其恢复稳定仍是体现其动态稳定性的一个重要性能指标。为此，本文基于有限时间控制原理，结合基于BBMC的异步电机调速系统的具体控制要求，提出一种基于有限时间控制（FTC）的变频调速控制方法；其基本原理是：根据异步电机的给定转速，经基于 PI-IP 控制的矢量控制算法获得 BBMC 的参考输出电压；再以BBMC 中电容电压与电感电流作为系统控制变量，经有限时间控制算法使BBMC 实际输出电压与上述参考输出电压保持一致，从而使异步电机实际转速实现对其给定转速的准确跟踪，达到了对BBMC调速系统实施高性能调速控制的目的。同时，由于该调速系统的相关性能指标与BBMC 主电路参数及有限时间控制参数均有关，因而提出采用自适应狼群优化算法对上述参数进行优化，取得了满意的效果。
...
文中建立了BBMC调速系统的数学模型，研究了调速系统基于有限时间控制的具体实现方法及相关参数的优化设计方法，并通过仿真和实验对其效果进行了验证；结果表明，基于BBMC的异步电机调速系统采用有限时间控制方法具有稳态跟踪误差小、动态性能好、抗干扰能力强等优点。
