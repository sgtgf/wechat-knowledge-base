# 文章精选 I 美国普渡大学印第安纳波利斯校区Haitham Kanakri等：用于DC-AC逆变的无电容三相固态功率滤波器（SSPF）

原创 CES TEMS编辑部 CES电机与系统学报 2026-03-02 07:00 北京
...
可靠性是电力电子领域的一个持续挑战，组件故障会严重影响系统性能。电容器广泛用于功率转换器的滤波，导致约30%的故障，主要是由于电化学腐蚀导致电容退化和灾难性故障。本文提出了一种用于三相逆变器的新型无电容固态电力滤波器（SSPF），为缓解与传统电感电容器（LC）和有源输出滤波器（AOF）相关的可靠性问题提供了一种变革性方法。与依赖于紧凑LC结构的AOF不同，SSPF完全消除了电容器，避免了其固有的故障模式。SSPF利用先进的固态器件和变压器技术，实现了卓越的滤波性能，提高了系统可靠性，并显著减少了元件数量，利用了AOF所需金属氧化物半导体场效应晶体管（MOSFET）开关的一半。这种设计不仅降低了成本，而且提高了效率。仿真和实验结果证明了SSPF在基频下提供正弦输出电压的能力。这些特性将SSPF定位为现代电力电子应用的稳健、经济高效和创新的解决方案。  
