# 技术群里的小伙伴们忧心忡忡: AI技术飞速发展，真的会取代硬件工程师吗？

原创 王工 硬件笔记本 2025-03-07 08:00 四川

> 原文地址: [https://mp.weixin.qq.com/s/ozWJSNeGEADFvA7d6AoOLg](https://mp.weixin.qq.com/s/ozWJSNeGEADFvA7d6AoOLg)

![图片](https://mmbiz.qpic.cn/mmbiz/cZV2hRpuAPiaJQXWGyC9wrUzIicibgXayrgibTYarT3A1yzttbtaO0JlV21wMqroGYT3QtPq2C7HMYsvicSB2p7dTBg/640?wx_fmt=gif&wxfrom=13&tp=wxpic "音符")点击上方名片关注了解更多![图片](https://mmbiz.qpic.cn/mmbiz/cZV2hRpuAPiaJQXWGyC9wrUzIicibgXayrgibTYarT3A1yzttbtaO0JlV21wMqroGYT3QtPq2C7HMYsvicSB2p7dTBg/640?wx_fmt=gif&wxfrom=13&tp=wxpic "音符")

  

大家好，我是王工。

最近技术群里小伙伴们有两个话题引起了我的思考：

1、热议AI，AI技术飞速发展，咱们工程师真的会被取代吗？本科应届生表示很担忧。  

2、有硬件工程师已跑路，准备转行跑外卖，吃饭的家伙都不要了，在群里大甩卖示波器，万用表，电烙铁，热风枪等。

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_002_7da97ddc6b98.jpg)

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_003_164026f7e426.jpg)

AI很强大，咱们工程师会不会被AI取代？何以稳定军心？咱们行业的出路和方向在哪？王工结合自己的经历，谈谈个人的一些看法。

  

01

**AI目前的趋势**

2025年是AI爆发的一年，各个平台到处都在大肆宣传AI的强大，甚至有些公共场所的AI机器人已经替代人类，完成一些简单的工作。

**举例1：**

最近，深圳的人形机器人辅助警察上街“巡逻”、基于DeepSeek开发的“AI数智员工”也开始辅助政务工作。

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_004_aa51eb9bee30.png)

这是真正的AI机器人，已经走进我们的现实生活。

  

**举例2：**

还有一个例子，咱们国家出手了，据《人民日报》报道，养老机器人或将代替子女和护工走进现实。

近日，国际电工委员会（IEC）正式发布**由我国牵头制定的养老机器人国际标准（IEC 63310《互联家庭环境下使用的主动辅助生活机器人性能准则》）**。该项标准依据老年人生理和行为特点，为各类养老机器人的产品设计、制造、测试和认证等提供基准，将引领全球养老机器人产业健康发展。

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_005_dfea5a81d4e3.png)

中国乃至全球，人口老龄化加重，如何让老年人更好的被照顾是一个社会课题。养老机器人能大大减轻社会和家庭对老人照顾的负担，让老年人有尊严的独立居家高质量生活。

  

**举例3：**

AI也能帮助我们搜集一些简单的素材，写工作汇报或者PPT，极大的提高了我们的工作效率。

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_006_00d65025f4ba.jpg)

  

02

**AI会取代硬件工程师吗？**

我先不回答这个问题，王工结合自己多年做硬件项目的经验，咱们一起来看看硬件工程师的工作内容。

首先强调，**硬件工程师不止研发硬件**，要做好一名硬件工程师，考验的是工程师本人的综合能力。

**1、对接项目经理：**

项目经理说：我这里来了一个新的订单，有XX需求，要求成本不能太高。这时候，硬件工程师需要做哪些工作呢？

-   项目的可行性研究；
    
-   尽量用公司的标准品改制，用现有物料，减少物料的种类，方便仓库管理；  
    
-   方案对比，找出最经济可行的方案；
    
-   跟项目经理反复磨合，一方面方案要合理且公司能够做到，另一方面要征得客户的同意。
    

  

**2、对接软件：**

假如我们要点亮一个LCD，软件工程师需要硬件的配合，并告知硬件选型设计的参数：

-   LCD本身支持的分辨率（如1920x1080），刷新率（如60Hz），水平/垂直同步信号（HSYNC/VSYNC），色彩深度（如RGB565或RGB888）；
    
-   接口类型：RGB，eDP，LVDS还是MIPI；  
    
-   GPIO的引脚配置：需确认GPIO引脚连接（如复位，使能，背光控制PWM控制引脚），需确认高低电平，以及PWM频率和占空比。
    
-   调试遇到问题时，需要准备万用表，示波器等基本工具逐一排查，确认是硬件问题还是软件问题。
    

  

**3、对接结构：**

假如我们要设计一个客户定制的主板板卡，硬件需要提供以下信息：  

-   电子元器件的尺寸规格书，安装位置建议，接口位置（如USB、电源、以太网、HDMI接口）；
    
-   需提供发热元件（如CPU、电源模块）的功耗，以便散热器的选型；
    
-   需标识敏感电路（如射频模块），以便结构工程师设计金属屏蔽罩以减少电磁干扰；
    
-   需提供天线类型（如PCB天线、外置天线）和安装位置。
    

以上只写了一小部分内容，实际需要跟结构工程师反复磨合，如果不合适需要重新选型。

  

**4、对接认证：**

关于认证，这个话题就太多了，它是咱们产品设计完成后的最后一关，也是相当复杂的一项工作。

最头痛的就是RE辐射测试，我认为这也是硬件工程师工作范围之一，也是最考验人的地方，这里就不展开讲了。

你以为这就完了？硬件工程师还要对接采购，工艺，生产，售后。总之，产品从设计到出货，你的项目有问题就会找到你。

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_007_887f205c490e.png)

大公司分的很细，就像我上面描述的，小公司可能一个人把硬件，PCB，软件，结构，认证，测试都给干完了。  

**综上所述，我认为硬件工程师这个岗位是具有创造性的，不仅仅是一些简单的操作，而是一整套带主观意识的系统。**  

如果硬件工程师失业了，那软件工程师呢？现在的AI已经可以写代码了。

那测试，结构，采购，测试，生产呢？你告诉我他们怎么办？

  

**结论：**

**所以，我认为硬件工程师不会被AI完全替代，但咱们的工作方式可能会发生变化。通过学习和适应新技术，AI将成为咱们的有力工具，帮助咱们在设计和开发过程中实现更高的效率和创新。**

**备注：**

1、以上我说的是一个相对复杂的产品，需要各个岗位的密切配合，相比之下，那些产品功能简单，对硬件工程师的要求不太高的行业就比较难说了。

2、所以，虽说都是硬件工程师，所选的行业，所做的产品不一样的话，工资待遇也是不一样的。

**咱们行业的出路和方向在哪里？那些知名的行业大佬就比较有发言权了。**

  

03

**咱们行业的出路和方向**

王工了解到，本月上海有一个比较大型的线下研讨会，会议主题直击**人工智能AI，5G 通信和量子计算芯片**等前沿热点，它能更好的指导咱们硬件工程师未来的一个就业方向，也能让各企业即时掌握国际先进技术与战略讯息。

这次会议来的都是行业大咖，包括：

-   ADl
    
-   Cadence
    
-   是德科技
    
-   OnSemi
    
-   Vishay
    
-   lmagination
    
-   芯原微电子
    
-   达摩院
    
-   腾讯云
    
-   思特威
    
-   思尔芯
    
-   东芯半导体
    
-   普冉半导体
    
-   英韧科技
    
-   芯成半导体
    
-   晶华微
    
-   英诺达
    
-   WeEn
    
-   ST半导体
    
-   阳光电源
    
-   贝岭  
    

以上只列举了部分公司，活动非常有意义，我建议大家有时间的话都尽量去现场看一看。

![图片](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_008_7d2fbd9b0e0e.png)

**活动日程**

![图片](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_009_7d2fbd9b0e0e.png)

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_010_da1196cf7193.png)

  

我特意为**咱们公众号硬件笔记本的粉丝申请了100个名额**，可点击如下资料免费报名，里面详细说明了**会议时间**，**地****点及****主题**。

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_011_195003f13029.png)

扫码免费预登记

如果你不确定是否报名成功，也可加王工微信woniu26a，我让他们工作人员帮忙给看一看。

  

**还有机会参与以下礼品的赠送**

![](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_012_e548481d19da.png)

-   小米智能摄像机
    
-   小度智能音箱
    
-   SAST 车载迷你冰箱
    
-   京东京造折叠自行车
    
-   京东京造星朗天文望远镜
    
-   荣耀路由器
    
-   米家高速吹风机
    
-   纽曼录音笔
    
-   罗技键鼠套装
    
-   SOO 复古行李箱
    
-   安壳折叠电热水壶
    
-   人体工学腰靠
    
-   卡蛙家用烘干机
    
-   健身摇摇杯
    
-   康佳电煮锅
    
-   京东京造露营睡袋
    
-   京东京造智能体脂秤
    
-   办公室卡通坐垫
    
-   多功能收纳包
    

以及其它免费小礼品

  

再补充一点，不得不说主办方也是相当的厉害

**会议主办方：**AspenCore

**主办方简介：**专注于**电子工程和半导体行业的技术媒体与信息服务**，提供高质量的行业新闻、技术文章、市场分析和产品评测，涵盖半导体、电子设计、嵌入式系统等领域，成为工程师和企业获取行业资讯和技术资源的主要渠道。

  

如果这篇文章对你有帮助，别忘了点赞、收藏，并分享给更多需要的人！

参考链接

https://baijiahao.baidu.com/s?id=1825285629652166017&wfr=spider&for=pc

https://mp.weixin.qq.com/s/A0xUHgxmXEmQEog19-FAwQ

  

  

  

推荐阅读（点击图片直接进入）

[![图片](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_013_8f872b8ba8ee.png)](https://mp.weixin.qq.com/s?__biz=Mzk0NjI3NzMwOQ==&mid=2247500681&idx=1&sn=117bdaa8c04eecdda16fb1d0c0e9fa37&scene=21#wechat_redirect)

[![图片](D:\电脑文件\公众号知识库\电工_行业资讯\技术群里的小伙伴们忧心忡忡__AI技术飞速发展_真的会取代硬件工程师吗__images\img_014_ae4afad92afd.png)](https://mp.weixin.qq.com/s?__biz=Mzk0NjI3NzMwOQ==&mid=2247505291&idx=1&sn=2a9d3e27af00369a4b4abec91356fb55&scene=21#wechat_redirect)

投稿/招聘/推广/宣传/技术咨询 请加微信：woniu26a

  

## 

**声明：**

  

声明：原创文章，转载请注明出处。本号对所有原创、转载文章的陈述与观点均保持中立，推送文章仅供读者学习和交流。文章、图片等版权归原作者享有，如有侵权，联系删除。

**推荐阅读▼**

-   [电路设计-电路分析](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=Mzk0NjI3NzMwOQ==&action=getalbum&album_id=2811359150088683521#wechat_redirect)  
    
-   [EMC相关文章](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=Mzk0NjI3NzMwOQ==&action=getalbum&album_id=2035870297278545920#wechat_redirect)
    
-   [电子元器件](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=Mzk0NjI3NzMwOQ==&action=getalbum&album_id=2035859110969114626#wechat_redirect)