...
原创 自动化学报 2019-11-01 11:45 北京

> 原文地址: [https://mp.weixin.qq.com/s/tWo1UGO3D\_8Tjw1U5q61hg](https://mp.weixin.qq.com/s/tWo1UGO3D_8Tjw1U5q61hg)
...
霍其润, 李建武, 陆耀, 秦明. 基于变分的CT图像环形伪影校正. 自动化学报, 2019, 45(9): 1713-1726.

  

环形伪影的形成主要与系统扫描方式和成像原理相关，如图1所示，自第三代CT系统至今，探测器的尺寸足够大，一次扫描即可实现整个物体投影数据的获取，对物体360度不同角度的扫描即可完成整个检测过程。
...
若某探测器损坏，各个角度的扫描产生的投影数据均会在相应位置出现缺损，从而投影正弦图上表现为一条异常直线，重建后的图像就会出现一个明显的单环伪影。对于这种情况通常可以通过硬件修复等方法实现有效地处理。

  

实际系统中，在探测器均能正常工作的情况下，由于加工工艺限制以及后续使用的损耗，各探测器的响应灵敏度很难实现完全一致，这些微小的差异，也会造成投影数据的偏差。所有角度的投影数据都有类似的偏差，基于相同的原理，重建后的CT图像中产生很多幅值较弱的环形伪影，如图2所示。
...
3)分析并提出环形伪影与图像结构性信息的边缘特性差异，模型中引入对伪影信息更有针对性抑制作用的相对全变分约束形式，从而增强模型对图像结构性信息的保护。

  

通过有效的优化算法实现模型求解，满足校正算法的实用性和准确性，并应用于实际的医学图像后处理中进行了有效性验证。
...
E-mail: qinming@epri.sgcc.com.cn
