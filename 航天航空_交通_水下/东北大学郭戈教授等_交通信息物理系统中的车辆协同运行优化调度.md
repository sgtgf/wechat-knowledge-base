# 东北大学郭戈教授等：交通信息物理系统中的车辆协同运行优化调度

原创 自动化学报 2019-04-16 09:00 北京

> 原文地址: [https://mp.weixin.qq.com/s/6vGudlec91EmPFpKb\_DDdw](https://mp.weixin.qq.com/s/6vGudlec91EmPFpKb_DDdw)

**车辆协同运行优化调度是指：**在交通信息物理系统的框架下，共享车辆的速度与位置信息，在保证准时到达的前提下，通过改变车辆的路径与速度的方式，实现车辆的协同编队行驶，达到节能减排的目的，提升道路的吞吐量。
...
集中式车辆调度系统是交通运输的发展趋势。目前，几乎所有车辆都采用导航推荐，驾驶员自主选择的驾驶方式，主要依托GPS定位及路径规划算法来实现路线推荐。然而，随着经济的发展，特别是考虑到用户需求及环境问题，这种路线推荐方法已经无法满足当前需求。例如：

  

1）道路分配不均问题：部分路段拥堵，部分路段空闲；

2）道路吞吐量问题：单位时间内通过道路的车辆最大数量少；

3）污染问题：车辆自由驾驶燃料利用不完全;

……

  

面对这些问题，当前的技术难以满足各种需求，因此亟需建立一个完备的交通调度系统，这是支撑大范围“交通调度”的使能技术。

  

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH44y9eTzBq5akbvQfucY0SicvSFNch4uYiabJ7NKH0LTNcuia8dgF6OTl7CJP3oGQKDNWqEFUfbzOOuxg/640?wx_fmt=png)

  

要实现大范围的交通调度，车辆必须配备有先进的信息处理和通讯设备,可实现车辆与车辆、车辆与设备通信。同时，云计算和服务框架提供了强大的计算及存储能力，并可与第三方工具和服务无缝集成，使得车辆的实时协调和自动控制成为可能。

  

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH44y9eTzBq5akbvQfucY0Sicvr4uRryYpRLU5UJNic4UIeyHxsfgvwu6PtrVAqPh9b1KIWv0YG24K0iaw/640?wx_fmt=png)

  

在车辆能够自动控制的前提下，还需要考虑到环境问题，为此采用协同运行的方式（即小间距列队行驶）以减少跟随车辆的空气阻力，从而降低油耗。此种行驶方式从空气动力学及实验角度分析均成立。

  

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH44y9eTzBq5akbvQfucY0SicvemzLdicVSiaEibzjdibl4LHN5fDNjYNibAzO7tUJ6Y723K6g5ESEfGPm5Eg/640?wx_fmt=png)

  

**在基础条件都具备的前提下，如何实现车辆协同运行优化调度？**

  

首先需要构建一个车辆调度系统，该系统由两层组成,，物理层和网络层。物理层描述了交通环境约束下的车辆移动性，而网络层描述交通物理系统和通信网络的信息。

  

其次，实现车辆的运输方案规划，车辆通过先进的传感及通讯技术实时共享车辆的运输信息，然后通过中央处理器计算出每辆车的运输计划并发送回车辆。当有信息更新时，重复上述规划过程。
...
