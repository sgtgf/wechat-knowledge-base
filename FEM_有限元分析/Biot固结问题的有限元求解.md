...
![](Biot固结问题的有限元求解_images/img_000_504b9a0acd89.png)

点击上方「蓝字」关注我们

![](Biot固结问题的有限元求解_images/img_001_92b2749bca28.png)
...
![](Biot固结问题的有限元求解_images/img_002_e5aa8754c1fe.png)
...
![](Biot固结问题的有限元求解_images/img_003_da7608737216.png)
...
![](Biot固结问题的有限元求解_images/img_004_729ef17644ec.gif)

**孔压的演化过程**

![](Biot固结问题的有限元求解_images/img_005_c55051bb4b7f.gif)

**不同深度处位移随时间的变化曲线**

土体表面（蓝线）和 0.5 m深处（红线）的位移情况如下图所示，反映了外力作用下土体逐渐压缩的过程。

![](Biot固结问题的有限元求解_images/img_006_7b725617cce1.png)

**不同高度处孔压随时间的变化曲线**

选取深度为 0.1、0.3、0.5、0.7 和 0.9 m 的位置作为观察点，孔压随时间变化的情况如下图所示。

![](Biot固结问题的有限元求解_images/img_007_c1d34f2b0ef4.png)
...
![](Biot固结问题的有限元求解_images/img_008_2c762fc0acb1.png)

### 前处理

进入 GiD 进行前处理。根据圆截面的对称性，取四分之一的区域进行模拟。网格剖分采用 8 节点等参单元。共使用了 300 个单元，961 个节点。

![](Biot固结问题的有限元求解_images/img_009_a97f67845005.png)
...
![](Biot固结问题的有限元求解_images/img_010_37701bba1a00.gif)

**孔压的演化过程**

![](Biot固结问题的有限元求解_images/img_011_d52a30635e25.gif)

**径向位移随时间的变化曲线**

土体表面（红线）和 0.5 m 深处（蓝线）的径向位移情况如下图所示，反映了土体整体逐渐压缩的过程。

![](Biot固结问题的有限元求解_images/img_012_9f88b70e22d4.png)

**孔压随时间的变化曲线**

选取径向距离 0.0、0.4 和 0.8 m 的三个位置作为观察点，孔压随时间变化的情况如下图所示。

![](Biot固结问题的有限元求解_images/img_013_6bd61a683d0e.png)
...
![](Biot固结问题的有限元求解_images/img_014_6dcdc46c2593.png)
...
![](Biot固结问题的有限元求解_images/img_015_039c938f471d.png)

**喜欢****作者******，请点********赞********和在看******![](Biot固结问题的有限元求解_images/img_016_90d3201b3d1c.gif)**

**![](Biot固结问题的有限元求解_images/img_017_2d172958cdc0.gif)**