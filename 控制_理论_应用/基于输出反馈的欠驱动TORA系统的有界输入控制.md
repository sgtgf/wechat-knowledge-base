...
原创 自动化学报 2020-04-24 17:30 北京

> 原文地址: [https://mp.weixin.qq.com/s/zyBT7jRgI27tLi3yjOgpdQ](https://mp.weixin.qq.com/s/zyBT7jRgI27tLi3yjOgpdQ)
...
武宪青, 徐可心, 张益波. 基于输出反馈的欠驱动TORA系统的有界输入控制. 自动化学报, 2020, 46(1): 200-204.

  

**如何同时解决TORA系统的输入饱和约束、输出反馈和旋转小球循环行为**？目前，国内外已有多位学者对TORA系统的控制问题进行了研究。常见的控制器设计方法有：一类是基于全状态反馈的控制方法；另一类是基于输出反馈的控制器设计方法。然而大部分现有文献均未考虑TORA系统中旋转小球可能存在的循环行为，即旋转小球的旋转角度θ=π和θ=2nπ在实际物理空间中代表相同的物理形态，而现有大部分控制器都无法辨别θ=π和θ=2nπ的区别。在实际情况中，控制器可能存在饱和约束问题，如果控制输入超过控制器可允许的最大输入，可能会导致控制器失灵，甚至影响系统的稳定性。
...
鉴于此，考虑到执行器可能存在饱和约束情况以及旋转小球可能出现的旋转现象，为了避免使用速度信号，本文针对TORA系统的镇定控制问题，提出了一种输出反馈控制器。具体而言，首先根据TORA系统模型分析了TORA系统的控制目标；随后，构造了一种新颖的能量函数，在此基础上设计了一种考虑执行器饱和约束的输出反馈控制器，并通过严格的数学分析证明了闭环系统关于平衡点的稳定性；最后，借助数值仿真测试检验了所提控制器的控制性能，并与已有方法进行了对比。
