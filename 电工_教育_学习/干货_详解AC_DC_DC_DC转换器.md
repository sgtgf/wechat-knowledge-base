...
> 原文地址: [https://mp.weixin.qq.com/s/GbYZPLwExhGfs-7ALfaL5Q](https://mp.weixin.qq.com/s/GbYZPLwExhGfs-7ALfaL5Q)
...
因此可以说，与不利用输入负电压成分的半波整流相比，全波整流是更具高效率的整流方法。此外，平滑后的纹波电压根据电容器容量和负载（LOAD）而变化。
...
DC/DC转换器是一种将DC（直流）转换为DC（直流）的元件，具体是指利用DC（直流）转换电压的元件。（关注公众号：硬件笔记本）IC等电子元件各自的工作电压范围不同，因此需要转换为相应的电压。
...
若只有正侧电源，（关注公众号：硬件笔记本）则不能处理接地电位以下的电压，不能将晶体管的输出引脚的电压分配至负电平。将控制晶体管装入负输出线，从而产生负电压。
...
这样，LDO在输出与标准型稳压器相同的电压时，（关注公众号：硬件笔记本）也可设定较低的输入电压。
