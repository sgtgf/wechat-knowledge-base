# 席磊等：分布式多区域多能微网群协同AGC算法

原创 自动化学报 自动化学报 2020-12-09 17:47 北京

> 原文地址: [https://mp.weixin.qq.com/s/NioKjfoEaQW6JxJVKSOzkg](https://mp.weixin.qq.com/s/NioKjfoEaQW6JxJVKSOzkg)
...
_http://www.aas.net.cn/cn/article/doi/10.16383/j.aas.c200105_

**1.  研究背景**

自动发电控制(AGC)是调节有功功率的重要手段，是实现电力系统频率无差调节的关键，原理框架如图1。随着电力系统规模的不断增加和电力市场的蓬勃发展，多区域互联电网已成为一种趋势。同时，伴随着微电网的不断发展，局部区域内多个微电网互联成微网群，有利于提高整体运行稳定性和经济性。而研究微网群的协同控制方法，有利于降低控制系统计算压力，保护微网群主体的信息安全。
...
**2.   面向多能微网群多智能体强化学习AGC协同控制算法的提出**

目前，AGC总功率跟踪控制器常采用经典PI控制或人工智能算法。其中，不依赖模型的强化学习AGC控制器跟踪效果和在线学习能力较为突出。故众多学者将强化学习算法引入AGC控制系统，如：Q、Q(λ)、SARSA和分层Q等单智能体算法。
...
由于电源种类繁多，能量密度低，相较传统电力系统，微网系统具有更强的分散性和不确定性。而微网群作为微电网概念的延伸，其较微电网有着更高的控制架构。对于微网群多智能体算法的分布式控制策略而言，如何利用多智能体智能算法来实现微网负荷频率的快速分布式协同控制成为了亟待解决的难点之一。

  

本文从AGC角度面向多区域多能微网群提出一种基于多智能体强化学习的频率控制方法，即DQ(σ,λ), 能够在避免传统强化学习动作探索值高估的同时, 实现分布式多区域的协同控制，其协同控制架构如图3。
...
时间差分学习(TD)是无模型强化学习中最重要的策略之一, TD方法结合了蒙特卡罗方法和动态规划的优点, 适用于无模型、持续进行的任务。Q(σ)通过引入采样参数σ, 统一了Sarsa算法和Expected-Sarsa算法，结合了在策略与离策略算法的优点。

  

以离散时间马尔可夫决策过程为数学基础, 基于Q(σ)算法并融入资格迹, 提出了一种新颖的快速多步算法Q(σ, λ), 以解决Q(σ)的时间信度分配问题, 进而提高AGC机组功率调节快速性。其TD目标是Sarsa和Expected-Sarsa的加权, 其中参数σ为控制权重。同时, 为了解决策略探索过程中动作值高估问题, 在Q(σ, λ)的基础上采用去耦“动作选择”和“动作评估”相结合的双重学习, 进而形成Double Q(σ, λ), 简称DQ(σ, λ)。

**4.   奖励函数设计**

DQ(σ, λ)智能控制器所追求的是CPS控制长期收益最大、尽可能避免频繁大幅度功率升降调节等目标，故本文奖励函数以CPS、ACE、频率偏差作为综合目标函数以获取最优AGC机组出力, 进而在最优AGC控制策略下达到系统功率平衡。

**5.   仿真效果对比**

本文根据南方电网相关数据搭建了融入大量新能源的分布式多区域多能微网群协同AGC模型, 以验证DQ(σ, λ)的实际工程应用效果。考虑到众多新能源的间歇性和强随机性, 通过引入随机负荷信号模拟真实电网环境中所面临的不确定性,实验指标与结果见表1，DQ(σ, λ)均优于其他算法，能够对分布式多区域多能微网群进行协同控制。
...
为了对综合能源模式下的分布式多区域微网群进行协同控制, 本文搭建了融入大量分布式能源的分布式多区域多能微网群协同的AGC模型, 并提出了一种多智能体协同控制DQ(σ, λ)算法，能够解决强化学习的时间信度分配与策略探索过程中动作值的高估问题，仿真结果验证了所述算法的有效性，能获得分布式多区域多能微网群协同运行。
...
三峡大学副教授、博士生导师, 2016年于华南理工大学获得博士学位. 主要研究方向为电力系统运行与控制、自动发电控制、智能控制方法的研究。
