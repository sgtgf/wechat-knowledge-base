...
![](D:\电脑文件\公众号知识库\电力电子_宽禁带器件_SiC_GaN\SiC_二极管在_PFC_应用中的简化损耗计算方法_images\img_000_ada8fa6621d6.png)
...
![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_001_17f0ccffbcd1.png)
...
![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_002_3aae8627aef6.png)

式中：VF 为二极管的正向导通压降，ton为二极管的开通时间，Vout为 PFC 输出电压，Q RR为二极管反向恢复电荷，由于SiC二极管Q RR\=0，因此该项可忽略。根据 BOOST 电路的结构，根据电感的伏秒平衡特性，可以得到以下关系：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_003_8197280bcbe1.png)

式中：Doff为开关管的关断的占空比，而开关管的关断占空比既为输出二极管的开通占空比。因此有：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_004_76249d38008a.png)

式中：T 为单个开关周期的时间，将（2）、（3）代入（1）可得工频周期内输入电压最高时的开关周期损耗为：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_005_22e41e2bcfde.png)

而由于输入电压Vin为正弦半波（正弦波经过整流桥变成正弦半波），因此单个开关周期内的输入电压：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_006_53393f82b661.png)

而由于每个周期内的平均电流在 PFC 的作用下追随输入电压，因此单个开关周期内的平均电流：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_007_12613771c8a2.png)

从而得到任意开关周期内的二极管平均损耗：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_008_8c7ad9a29ee1.png)

因此，对一个工频周期内的所有开关周期的二极管平均损耗进行积分，得到工频周期内的二极管平均损耗：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_009_7b2446e3b311.png)

而工频周期内二极管的平均功耗：

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_010_cb48d13abcd1.png)
...
![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_011_5fb12282e89d.png)

注意，Si二极管因为有较大的反向恢复电荷QRR，其由于反向恢复特性带来的损耗部分不能忽略， 因此该结论不适用于普通的 Si 二极管。

4. 实验验证

![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_012_284ad8506a58.png)
...
![](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_013_e81cfe2d2a2c.png)
...
![图片](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_014_60fa0102061e.jpg)
...
![图片](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_015_f64ad9e9a6fc.jpg)

![图片](SiC_二极管在_PFC_应用中的简化损耗计算方法_images/img_016_c68336bd29cb.png)