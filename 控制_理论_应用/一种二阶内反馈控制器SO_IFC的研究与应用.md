...
原创 自动化学报 2019-06-21 17:30 北京

> 原文地址: [https://mp.weixin.qq.com/s/WKZiMaR7sSqbPQoY9QEv\_A](https://mp.weixin.qq.com/s/WKZiMaR7sSqbPQoY9QEv_A)

二阶内反馈控制器是指：通过高增益PID控制器的逆变换作用，将一种新型内反馈器的结构进行反转，构造出一种新型控制策略，对高阶难控过程具有较好的控制性能。

  

李军, 万文军, 刘哲. 一种二阶内反馈控制器SO-IFC的研究与应用. 自动化学报, 2019, 45(5): 993-1003.

  

工程师提出了一种基于高增益PID控制器和内反馈器构造的一阶内反馈控制器。为了进一步提升内反馈控制器的控制性能，提出了一种基于高增益PID控制器和新型内反馈器构造的二阶内反馈控制器。高增益PID控制器是新型内反馈器的一种逆变换器，将新型内反馈器反转，构造出一种新型控制策略。

  

通过二阶内反馈控制器、输入函数发生器、过程对象、新型正弦跟踪滤波器、扰动模型等构造出一种二阶内反馈控制系统。
...
输入函数发生器用于简单的过程轨迹控制，两级新型正弦跟踪滤波器用于滤除高频随机干扰。新型正弦跟踪滤波器是一种“高效率”的线性滤波策略，在滤除高频随机干扰的同时，具有输出跟踪输入滞后较小的特性，并且跟踪特性与输入幅值无关，具有其他滤波策略不可替代的重要作用。

  

新型内反馈器由一阶微分器和一阶惯性环节所构成。
...
将二阶内反馈控制器运用于某1 000MW超超临界燃煤机组的锅炉主控系统的优化。优化前，所述锅炉主控系统采用PID控制。
...
采用PID控制器控制，锅炉主控输出的波动较大。在稳态负荷条件下，锅炉主汽压力跟踪锅炉压力给定的最大误差为0.51MPa。在变负荷条件下，锅炉主汽压力跟踪锅炉压力给定的最大动态误差为1.17MPa。

  

相对而言，采用二阶内反馈控制器控制，锅炉主控输出变化平稳，能有效克服外界扰动。在稳态负荷条件下，锅炉主汽压力跟踪锅炉压力给定的最大误差为0.28MPa。在变负荷条件下，锅炉主汽压力跟踪锅炉压力给定的最大动态误差为0.68MPa。可见，采用二阶内反馈控制器在所述锅炉主控系统优化收到了明显的效果。
