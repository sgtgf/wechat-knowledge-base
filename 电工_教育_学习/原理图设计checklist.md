...
> 原文地址: [https://mp.weixin.qq.com/s/7lHVZ77vjecEg0Nj4-RIYg](https://mp.weixin.qq.com/s/7lHVZ77vjecEg0Nj4-RIYg)
...
官网资料：https://www.nordicsemi.com/Products/nRF51822
...
GND和AGND电路要分开,但最后要通过一点进行连接。如果是chipset的    AGND电流很大，可直接与GND相连，不需要连接0OHM,否则通流不够。
...
空余的没有使用的GPIO   Pin接到LED上，一般3-4个LED即可。
...
CPLD    core电和IO电时序，一般要求core电要早于IO电，否则，输出信号需要加下拉电阻。（一般情况下core电都早于IO电压，Core起来之后IO状态就可以固定了。具体要求参考厂家器件资料）

102

FPGA

FPGA的MGT   Bank如果不用时，RX信号需要接地处理。
...
高速信号连接器，定义信号时，注意TX，RX在连接器上的分布，避免TX/RX混在一起（避免cross   talk）
...
clock   signal（除differential  Signal外），要预留可调节EMI 的电容位置，一般为10pF.

128

时钟

PCI-E2.0   slot的clock  signal 建议与控制芯片同源。

129

时钟

当Clockgen或Clock   Buffer使用SYS供电时，应注意网卡、CPLD等芯片的时钟信号是否需要单独的时钟源

130

时钟

所有Clockgen和Clock   Buffer的SMbus接口上拉的电压应与IC的供电一致

131

时钟

当晶振或clock   buffer输出的电平和IC需要的电平不一致时需要加AC耦合和阻抗匹配电路，同时要注意SWING和CROSSPOINT设置是否正确。
