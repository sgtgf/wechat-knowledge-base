...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_000_fc521f0c5564.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_001_87f020755a6f.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_002_87a6852fb5a1.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_003_d4bbc88ec061.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_004_63909284a362.png)

式中：gf表示跨导系数；Uth表示阈值电压。

2. 特性参数测试平台

![](D:\电脑文件\公众号知识库\电力电子_宽禁带器件_SiC_GaN\SiC_MOSFET功率器件特性参数的提取与拟合_images\img_000_9a86f5c362ea.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_006_a2479dea505e.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_007_6e1637cfb59c.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_008_a7760e0d1f23.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_009_cad271722e97.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_010_a763f6039e16.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_011_fc1e06467769.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_012_911cb8d06890.png)

SiC MOSFET功率器件中的漏源极寄生电容Cgd、栅源极寄生电容Cgs和漏源极寄生电容Cds具有明显的非线性特征，其电容值随漏源极电压uds变化而变化。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_013_cafd8b9547b4.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_014_6d8f21292808.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_015_60bd9f2ec073.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_016_c659b2bed375.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_017_27b592330701.png)

求解出电感值，即引脚寄生电感值。需要指出的是，此方式提取的电容值是SiC MOSFET功率器件内部寄生电容在零偏置电压(即uds=0V)下的值，可与器件的数据手册提供的电容值比较，作为验证结果准确性的依据。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_018_166a66a6515d.png)

式中：Ls-g=Ls+Lg；Cs-g=Cs·Cg/(Cs+Cg)；Ld-g=Ld+Lg；Cd-g=Cd·Cg/(Cd+Cg)。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_019_ef9d92b3a6ab.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_020_7342e9730059.png)

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_021_87f708a32773.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_022_359c0878d3a1.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_023_39f95502a09d.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_024_01a6b30b32e1.png)

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_025_976eca78ae29.png)

式中：e表示误差函数；dij表示第i个样本输出层第j个神经元的期望输出；yij表示相应的实际输出。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_026_fc9cb77881b3.png)

式中：k为迭代次数；xk表示权重和偏置构成的一维向量；I为单位矩阵；μk表示组合系数；e(xk)表示误差函数；J(xk)为雅可比(Jacobian)矩阵，表示e(xk)对xk求一阶偏导数所构成的矩阵。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_027_21ce03cbe25b.png)

式中：F为损失函数；

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_028_2836e7605a54.png)

，表示网络权重平方和均值，α、β为比例系数，N为总权重个数。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_029_f4c92184c2ef.png)

式中：D表示训练集；M表示神经网络结构；P(x|D,α,β,M)表示权重x的后验概率；P(D|x,β,M)表示训练集D的概率密度；P(x|α,M)表示权重x的先验密度；P(D|α,β,M)表示训练集D的边缘概率，为归一化因子。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_030_d5b97cca8d9c.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_031_7bcdd35fb279.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_032_74ca0a55995e.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_033_6d6d4bc3df16.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_034_1780dd24d70a.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_035_4fdd04894279.png)

3.2.4体二极管

将SiC MOSFET功率器件内部的体二极管的伏安特性曲线看作关于结温Tj的函数，即uds=f(Tj,id)，使用BP人工神经网络进行拟合，其拟合结果如图21和表8所示，呈负温度特性。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_036_5d5722b92b04.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_037_44e7a2bbe1a0.png)

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_038_8a1f53928d00.png)
...
![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_039_318ead5d55de.jpg)

    专注碳化硅器件的研发与应用。分享碳化硅器件的设计@研发@应用等行业资料。

  加交流微信群，请添加个人微信：18126115420，并备注单位+姓名+研发方向。

![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_040_3ade3c3d8599.jpg)![](SiC_MOSFET功率器件特性参数的提取与拟合_images/img_041_84aa944feb13.jpg)