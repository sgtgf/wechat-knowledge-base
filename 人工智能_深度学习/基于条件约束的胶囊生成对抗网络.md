...
原创 自动化学报 2020-04-10 17:00 北京

> 原文地址: [https://mp.weixin.qq.com/s/VeOeoANstzTaU\_vpQXW6qA](https://mp.weixin.qq.com/s/VeOeoANstzTaU_vpQXW6qA)

  

  

基于条件约束的胶囊生成对抗网络是指：鉴于胶囊网络结构作为有别于传统CNN而提出的一种全新的神经网络结构，建立基于胶囊网络的GAN模型，细节为将胶囊网络作为GAN中判别器的框架，同时在网络中加入条件约束。完成对GAN传统模型的升级，从而提高GAN在训练和生成过程中的稳定性。
...
自2014年Ian Goodfellow提出了GAN（Generative Adversarial Network）以来，对GAN的研究可谓如火如荼。GAN在建模基于真实数据的高度复杂的分布（特别是图像）方面表现出色。从基于CNN（GAN）到基于CNN（DCGAN）再到基于RNN（GRAN），网络结构的创新和优化是GAN得以不断发展的重要原因之一。

  

而传统的CNN的内部数据表示并没有考虑到简单和复杂对象之间的重要空间层级，因此通过使用更具有优势的胶囊网络从而对GAN模型进行重构，我们希望促使GAN能够更完备地学习到真实分布，从而更有效地抑制模式坍塌情况的发生。我们的设计如下：

  

![](https://mmbiz.qpic.cn/mmbiz_jpg/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgD6iaNLiabOJJcicQ93V1plGSk2OMNSKgYWumBqnfw2beUFVICjGlPhhkicw/640?wx_fmt=jpeg)

  

作者在MNIST和CIFAR-10数据集上验证了网络的有效性，特别地，在 MINST 数据集上着重探索了胶囊个数对生成对抗网络的训练收敛性和生成质量的影响，得出胶囊个数对于生成结果来说存在最优值的结论；在CIFAR-10 数据集上则探索了本模型在彩色复杂数据集上的应用，并通过对比现有的生成对抗网络架构的训练过程和样本结果发现，本文方法在胶囊结构的支持下能够更加有效的抑制模式坍塌问题。以下是部分的生成结果。

  

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgDzbiadkPIOLV9CUEdTNmdARu3OQQLHX3aU2CLydFpuzpaBGfC4sUWBeA/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgDC8iciafyS7jOvGoxTEw9VO0uictxbZI4wgeSV0UO1tVTRsk9a8VxopDfQ/640?wx_fmt=png)

孔锐，暨南大学智能科学与工程学院教授。主要研究方向为图像识别。本文通信作者。 

E-mail: tkongrui@jnu.edu.cn

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgDVbaQ41yMREXCfQDWJ8xweNAFutJEYlVeObm3QQX6sh90iaWLGAVibwmg/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgDDd17KR8tEld9km6TX7HZYrtgrA12Acozq7LY4RjCNG2beaJ0rr9w2A/640?wx_fmt=png)

![](https://mmbiz.qpic.cn/mmbiz_jpg/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgDITsiartFAfmxVEJjXwhbToA3cFHWDR7DuphlibjlIHPfZY74sopfpibWw/640?wx_fmt=jpeg)

JAS《自动化学报》（英文版）

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgDDd17KR8tEld9km6TX7HZYrtgrA12Acozq7LY4RjCNG2beaJ0rr9w2A/640?wx_fmt=png)

![](https://mmbiz.qpic.cn/mmbiz_jpg/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgD5Fk9pg1o9ugVTrq5Y8ClmUwaM5l9ygKicGOYibylorjhGXqdXbHq72pQ/640?wx_fmt=jpeg)

自动化学报服务号

![](https://mmbiz.qpic.cn/mmbiz_png/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgDDd17KR8tEld9km6TX7HZYrtgrA12Acozq7LY4RjCNG2beaJ0rr9w2A/640?wx_fmt=png)

![](https://mmbiz.qpic.cn/mmbiz_jpg/47ibaBJyUH46ZTZeAEuD0XwHPODPuWQgD2UfNu9icJlhsjMmLj4cdQHUia3iazqjKibkbCftLwPhO7DQmOTciapZl5Xw/640?wx_fmt=jpeg)
